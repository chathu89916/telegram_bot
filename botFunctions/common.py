...
def setupFullName(name):
    firstName = name.first_name if name.first_name is not None else ''
    lastName = " " + name.last_name if name.last_name is not None else ''
    userName = " (@" + name.username + ")" if name.username is not None else ''
    fullName = firstName + lastName + userName
    return fullName


def getName(name):
    if name.username is None:
...
def exceptionHandling(message, bot, types, name):
    ...
    bot.send_message(message.chat.id,
                     text='<b>Agent</b> ' + getName(name) + ' <b>Click the START button to Activate the</b> @' + str(
                         configuration.botUsername) + " " + emojiList.successFaceIcon, reply_markup=inline,
                     parse_mode='HTML')
        ...
        for admin in bot.get_chat_administrators(chat_id=message.chat.id):
            try:
                bot.send_message(chat_id=admin.user.id, text='<b>Tell</b> ' + getName(
                    message.from_user) + ' <b>to START me in privately. This is important, otherwise I cannot send message to</b> ' + getName(
...
"""
    ...
    markup.add(types.InlineKeyboardButton("Yes", callback_data=callBackDetails),
...
def structureGroupDetails(bot, groupID):
    ...
    adminDetails = ''
    creatorDetails = ''
    if allDetails.description is None:
        description = ""
    else:
    ...
    for admin in bot.get_chat_administrators(groupID):
        if admin.status == 'creator':
            ...
            if not admin.user.is_bot:
                adminDetails = adminDetails + "\n\t\t\t" + emojiList.groupAdmindminIcon + " Administrator : " + setupFullName(
                    admin.user)
            else:
                adminDetails = adminDetails + "\n\t\t\t" + emojiList.botIcon + " Administrator : " + setupFullName(
                    admin.user)
