            ...
            for userid in botFunctions.getAllUsers(message.chat.id):
                if (botFunctions.memberInTheGroup(bot, message.chat.id, userid)):
                    try:
                        ...
                        bot.send_photo(chat_id=userid, photo=message.photo[-1].file_id, caption=message.caption, parse_mode='HTML')
                    except:
            ...
            for uname in listUser:
                if (botFunctions.memberInTheGroup(bot, message.chat.id, uname)):
                    ...
                    if(str(repliedUser)!=uname):
                        try:
                            bot.send_message(chat_id=uname, text=mentionedUser + ' @ <b>' + message.chat.title + '</b> : mention you in a photo', parse_mode='HTML')
                        except:
