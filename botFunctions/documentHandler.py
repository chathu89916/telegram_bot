                        ...
                        try:
                            bot.send_message(chat_id=repliedUser,
                                             text=botFunctions.getName(
                                                 message.from_user) + ' @ <b>' + message.chat.title + '</b> : reply as a Document',
                                             parse_mode='HTML')
                        except:
                            print('reply to document failed')
            ...
            for uname in listUser:
                if (botFunctions.memberInTheGroup(bot, message.chat.id, uname)):
                    ...
                    if(str(repliedUser)!=uname):
                        try:
                            bot.send_message(chat_id=uname, text=mentionedUser + ' @ <b>' + message.chat.title + '</b> : mention you in a document', parse_mode='HTML')
                        except:
