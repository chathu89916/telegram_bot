...
def privatePhoto(bot, message):
    if(message.from_user.id == admin and message.reply_to_message != None):
        try:
            bot.send_photo(chat_id=message.reply_to_message.forward_from.id, photo=message.photo[-1].file_id, caption=message.caption)
        except:
    ...
    if (common.checkAdmin(bot, message.chat.id, message.from_user.id)):
        if(message.chat.type != 'private'):
            ...
            for userid in dbFunction.getAllUsers(message.chat.id):
                try:
                    bot.send_photo(chat_id=userid, photo=message.photo[-1].file_id, caption=text)
                except:
    ...
    if (message.chat.type != 'private'):
        ...
        if(len(listUser)>0):
            ...
            for uname in listUser:
                try:
                    bot.send_photo(chat_id=uname, photo=message.photo[-1].file_id, caption=text)
                except:
